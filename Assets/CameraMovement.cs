...
public class CameraMovement : MonoBehaviour {
	...
	void Update () {
		...
		if (noZ) gameObject.transform.position = Vector3.SmoothDamp(gameObject.transform.position, new Vector3 (gameObject.transform.position.x, gameObject.transform.position.y - Input.GetAxis ("Mouse ScrollWheel") * 120, gameObject.transform.position.z), ref vitez, 0.2f);
		else gameObject.transform.position = Vector3.SmoothDamp(gameObject.transform.position, new Vector3 (gameObject.transform.position.x, gameObject.transform.position.y - Input.GetAxis ("Mouse ScrollWheel") * 120, gameObject.transform.position.z + Input.GetAxis ("Mouse ScrollWheel") * 120), ref vitez, 0.2f);
		...
		if (gameObject.transform.position.y >= 3f && gameObject.transform.position.y <= 20f)
			noZ = false;
		//SCROLLING END
